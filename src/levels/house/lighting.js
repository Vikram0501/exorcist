...
function addHauntedSky(level) {

  const sky = new THREE.Group()
  sky.name = 'haunted-night-sky'
  ...
  for (let index = 0; index < STAR_COUNT; index++) {
    ...
    const y = 0.16 + seededRandom(index * 5 + 2) * 0.82
    const horizontal = Math.sqrt(1 - y * y)
    const radius = SKY_RADIUS - 2 - seededRandom(index * 5 + 3) * 8
    const brightness = 0.45 + seededRandom(index * 5 + 4) * 0.55
  ...
  stars.name = 'cold-star-field'
  sky.add(stars)

  const moonTexture = new THREE.TextureLoader().load(
    '/textures/haunted-moon.png',
  )
  ...
  moon.name = 'pale-haunted-moon'
  moon.position.set(-34, 52, 95)
  moon.scale.set(19, 19, 1)
  sky.add(moon)
